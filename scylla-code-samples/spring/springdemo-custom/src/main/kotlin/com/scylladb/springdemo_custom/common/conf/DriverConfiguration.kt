...
class DriverConfiguration {
    private val log = LoggerFactory.getLogger(DriverConfiguration::class.java)
    ...
    fun sessionBuilder(
        ...
        for (contactPoint in contactPoints!!) {
            val add = contactPoint.split(":").toTypedArray();
            val address = InetSocketAddress.createUnresolved(add[0], Integer.parseInt(add[1]))
            log.info("Adding contact point: {}", address);
